...
void SquarePlots3x3RealLayer() {
    ...
    TFile *outputFile = new TFile("Plots_Squares/Plots3x3_RealLayers.root","RECREATE");
    outputFile->cd();
    ...
    fileName << "scripts/EffSigmaSquares/EffSigma_Truth3x3_RL.csv";
    ...
    TCanvas c1("c1");
    c1.Print("LayerBiases3x3_RealLayers.pdf(")
    for (unsigned layer(0);layer<28;layer++) {
    ...
    c1.Print("LayerCorrections3x3_RealLayers.pdf(");
    for (unsigned layer(0);layer<28;layer++) {
...
TH1F* getCorrectedSliceProjection(unsigned layer, unsigned numBins, TTree* tree, float min, float max, TF1 *fit) {
    ...
    for (unsigned event(0);event<tree->GetEntries();event++) {
        tree->GetEntry(event);
        float x_ew = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(1)->GetName())->GetValue(layer);
        float x_t  = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(3)->GetName())->GetValue(layer);
        float x_e  = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(7)->GetName())->GetValue(layer);
        bool  aCut = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(12)->GetName())->GetValue(layer);
        bool  cCut = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(14)->GetName())->GetValue(layer);
        float x_et = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(16)->GetName())->GetValue(layer);
