...
void SquarePlots1() {
    ...
    TFile *file = TFile::Open("out_Sq_V100.root"); 
    TFile *outputFile = new TFile("Plots_Squares/Plots1.root","RECREATE");
    outputFile->cd();
    ...
    fileName << "scripts/EffSigmaSquares/EffSigma_LS1" << startLayer << ".csv";
    ...
    TCanvas c1("c1");
    c1.Print(Form("LayerBiases1_S%d.pdf(",startLayer));
    for (unsigned layer(0);layer<28;layer++) {
    ...
    c1.Print(Form("LayerCorrections1_S%d.pdf(",startLayer));
    for (unsigned layer(0);layer<28;layer++) {
...
TH1F* getCorrectedSliceProjection(unsigned layer, unsigned numBins, TTree* tree, float min, float max, TF1 *fit, int startPick) {
    ...
    for (unsigned event(0);event<tree->GetEntries();event++) {
        ...
        float x_e  = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(16)->GetName())->GetValue(start + layer);
