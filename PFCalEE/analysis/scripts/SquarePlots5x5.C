...
void SquarePlots5x5() {
    ...
    TFile *outputFile = new TFile("Plots_Squares/Plots5x5.root","RECREATE");
    outputFile->cd();
    ...
    fileName << "scripts/EffSigmaSquares/EffSigma_Truth5x5_LS" << startLayer << ".csv";
...
struct TrackInfo {
    ...
    Float_t energyWeighted5x5X[28];     //18
    Float_t energyWeighted5x5Y[28];     //19
    Float_t distsFromTileEdges5x5X[28]; //20
    Float_t distsFromTileEdges5x5Y[28]; //21
...
TH1F* getCorrectedSliceProjection(unsigned layer, unsigned numBins, TTree* tree, float min, float max, TF1 *fit, int startPick) {
    ...
    for (unsigned event(0);event<tree->GetEntries();event++) {
        ...
        float x_ew = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(18)->GetName())->GetValue(start + layer);
        float x_t  = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(3)->GetName())->GetValue(start + layer);
        float x_e  = tree->GetBranch("truthInfo")->GetLeaf(leafNames->At(20)->GetName())->GetValue(start + layer);
